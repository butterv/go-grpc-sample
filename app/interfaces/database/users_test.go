...
func testUserOneToOneUserPasswordUsingUserPassword(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()

	var foreign UserPassword
	var local User

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &foreign, userPasswordDBTypes, true, userPasswordColumnsWithDefault...); err != nil {
	...
	check, err := local.UserPassword().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadUserPassword(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.UserPassword == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.UserPassword = nil
	if err = local.L.LoadUserPassword(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.UserPassword == nil {
		t.Error("struct should have been eager loaded")
...
func testUserOneToOneSetOpUserPasswordUsingUserPassword(t *testing.T) {
	var err error
	...
	var a User
	var b, c UserPassword

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &b, userPasswordDBTypes, false, strmangle.SetComplement(userPasswordPrimaryKeyColumns, userPasswordColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, userPasswordDBTypes, false, strmangle.SetComplement(userPasswordPrimaryKeyColumns, userPasswordColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*UserPassword{&b, &c} {
		err = a.SetUserPassword(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.UserPassword != x {
		...
		if exists, err := UserPasswordExists(ctx, tx, x.UserID); err != nil {
			t.Fatal(err)
		} else if !exists {
...
func testUserToManyUserTokens(t *testing.T) {
	...
	check, err := a.UserTokens().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	userDBTypes = map[string]string{`ID`: `varchar`, `Email`: `varchar`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`, `DeletedAt`: `datetime`}
	_           = bytes.MinRead
)
